...
from src.model import SimpleCNN2, SimpleCNN
...
def train(opt):
    ...
    envs = MultipleEnvironments(worldStage, opt.action_type, opt.num_processes)
    ...
    optimizer_BC = torch.optim.Adam(model.parameters(), lr=opt.lr_BC)
    [agent_conn.send(("reset", None, worldStage)) for agent_conn in envs.agent_conns]
    ...
    save_path = "{}/{}/{}-{}".format(opt.saved_path, 'OneStage', worldStage[0], worldStage[1])
    logger = Logger(log_dir=(save_path+"/tensorboard"))
    states_list = [[] for _ in range(opt.num_processes)]
    actions_list = [[] for _ in range(opt.num_processes)]
    ...
    while True:
        ...
        for _ in range(opt.num_local_steps):
            ...
            for index, act in enumerate(action.cpu()):
                states_list[index].append(np.array(curr_states[index,:,:,:].cpu()))
                actions_list[index].append(action.cpu()[index])
                envs.agent_conns[index].send(("step", act, None))
            ...
            for index, agent_conn in enumerate(envs.agent_conns):
                if done[index] == 1:
                    if info[index]['flag_get']:
                        state_list = np.stack(states_list[index])
                        action_list = actions_list[index]
                        if info[index]['time'] >= max_timeleft:
                            max_timeleft = info[index]['time']
                            np.savez(os.path.join(opt.data_path, "states_actions_{}-{}".format(worldStage[0], worldStage[1])), state_list=state_list, action_list=action_list)
                            print("saving success history | timeleft : {}s".format(max_timeleft))
                            torch.save(model.state_dict(),save_path+"/latest")
                    states_list[index] = []
                    actions_list[index] = []
